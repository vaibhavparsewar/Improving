...
const fs = require("fs"); //from nodejs
...
//!configure multer
const upload = multer({ dest: "upload/" }); //save file in local machine
app.use(express.json({ limit: "10mb" })); //to accept incoming json data

//!initialize Google Generative AI
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
app.use(express.static("public")); //configure server to serve static asset

//!routes analyze and download
//analyze
app.post("/analyze", upload.single("image"), async (req, res) => {
  //image should be same name given in postman>body>form key
  try {
    if (!req.file) {
      return res.status(400).json({ error: "No image file uploaded" });
    }

    const imagePath = req.file.path; //path is property in req.file url of the uploaded image
    //encode the image
    ...
    // Clean up: delete or remove the uploaded file
