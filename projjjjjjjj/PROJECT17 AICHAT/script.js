...
document.addEventListener("DOMContentLoaded", () => {
  ...
  chatForm.addEventListener("submit", async (e) => {
    ...
    addMessage(message, true); //?msg of user so true
    ...
    try {
      ...
      //Ad AI response
      addMessage(response, false); //?msg is response so not user so false
    } catch (error) {
      typingIndicator.remove();
  ...
  async function generateResponse(prompt) {
    const response = await fetch(
      `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=AIzaSyAm4g81xhp-lRoW8jercKzP1Z-0fuHPwp8`,
      {
        method: "POST",
        headers: {
